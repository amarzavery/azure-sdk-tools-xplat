...
exports.init = function (cli) {
  ...
  // https://ygstoragefordeploy.blob.core.windows.net/ygcontainerfordeploy/YGCloudService.cspkg
  var deployment = cloudService.category('deployment')
    .description($('Commands to manage your deployment to Cloud Services'));

  deployment.command('create [serviceName] [deploymentName] [slot] [packageUri] [serviceConfigurationPath]')
    .description($('Create a deployment to a cloud service'))
    .usage('[options] <serviceName> <deploymentName> <slot> <packageUri> <serviceConfigurationPath>')
    .option('--serviceName <serviceName>', $('the cloud service name'))
    .option('--deploymentName <deploymentName>', $('the deployemnt name'))
    .option('--slot <slot>', $('the deployment slot. Valid values are \'Production\' or \'Staging\'.'))
    .option('--packageUri <packageUri>', $('specifies the path or URI to a .cspkg in blob storage. The storage account must belong to the same subscription as the deployment.'))
    .option('--serviceConfigurationPath <serviceConfigurationPath>', $('specifies a .cscfg file on disk.'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function (serviceName, deploymentName, slot, packageUri, serviceConfigurationPath, options, _) {
      var service = utils._createComputeClient(profile.current.getSubscription(options.subscription), log);
      //serviceName, deploymentSlot, parameters, callback
      var serviceConfig = fs.readFileSync(serviceConfigurationPath, 'utf8');
      var parameters = {
      ...
      var progress = cli.interaction.progress($('Creating deployment to cloud service'));
      try {
        service.deployments.create(serviceName, slot, parameters, _);
      } finally {
        progress.end();
      }
    });
