...
__.extend(DnsZone.prototype, {
  ...
  list: function (options, _) {
    ...
    try {
      if (options.resourceGroup) {
        dnsZones = self.dnsManagementClient.zones.listByResourceGroup(options.resourceGroup, options, _);
        nextLink = dnsZones.nextLink;
        while (nextLink !== undefined) {
          self.output.silly('Following nextLink');
          nextZones = self.dnsManagementClient.zones.listByResourceGroupNext(nextLink, options, _);
          dnsZones.zones = dnsZones.concat(nextZones.zones);
          nextLink = nextZones.nextLink;
        }
      } else {
        dnsZones = self.dnsManagementClient.zones.list(options, _);
        nextLink = dnsZones.nextLink;
        while (nextLink !== undefined) {
          self.output.silly('Following nextLink');
          nextZones = self.dnsManagementClient.zones.listNext(nextLink, options, _);
          dnsZones.zones = dnsZones.concat(nextZones.zones);
          nextLink = nextZones.nextLink;
        }
      }
  ...
  clear: function (resourceGroupName, zoneName, options, _) {
    ...
    try {
      recordSets = self.dnsManagementClient.recordSets.listByDnsZone(resourceGroupName, zoneName, options, _);
    } finally {
      progress.end();
    }
  ...
  export: function (resourceGroupName, zoneName, options, _) {
    ...
    var recordSets = self.dnsManagementClient.recordSets.listByDnsZone(resourceGroupName, zoneName, options, _);

    var nextLink = recordSets.nextLink;
    while (nextLink !== undefined) {
      self.output.silly('Following nextLink');
      var nextRecordSets = self.dnsManagementClient.recordSets.listByDnsZoneNext(nextLink, _);
  ...
  listRecordSets: function (resourceGroupName, zoneName, options, _) {
    ...
    try {
      if (options.type) {
        ...
        dnsRecords = self.dnsManagementClient.recordSets.listByDnsZone(resourceGroupName, zoneName, options, _);
      }
    } finally {
      progress.end();
    }
